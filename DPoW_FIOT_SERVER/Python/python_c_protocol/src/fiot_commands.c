...
FPYC_ERR gen_rand_no_entropy(uint8_t *output, size_t output_len)
{
   ...
   if (fread(output, 1, output_len, f)^output_len)
      err=PyC_ERR_READING_FILE;
...
FPYC_ERR prepare_command(F_NANO_HW_TRANSACTION *buffer, void *raw_data)
{
   ...
   if ((strnlen((const char *)buffer->hdr.publish_str, F_NANO_MQTT_PUBLISH_STR_SZ))==F_NANO_MQTT_PUBLISH_STR_SZ)
      return PyC_ERR_PUBLISH_ADDR_OVFL;

   if (buffer->hdr.raw_data_sz>F_NANO_TRANSACTION_RAW_DATA_SZ_MAX)
      return PyC_ERR_F_NANO_TRANSACTION_RAW_DATA_SZ_MAX_OVF;

   buffer->hdr.preamble=F_PREAMBLE;
   buffer->hdr.version=F_F_NANO_TRANSACTION_VERSION;
   buffer->hdr.timestamp=(uint64_t)time(NULL);
   buffer->hdr.crc32=0;

   if ((err=gen_rand_no_entropy((uint8_t *)&buffer->hdr.last_msg_id, sizeof(uint32_t))))
      return err;

   sz_tmp=sizeof(F_NANO_TRANSACTION_HDR)+buffer->hdr.raw_data_sz;

   if (raw_data)
      memcpy((void *)buffer->rawdata, raw_data, buffer->hdr.raw_data_sz);

   crc32_tmp=crc32_init((unsigned char *)buffer, sz_tmp, 0);
...
static uint32_t crc32_init(unsigned char *p, size_t len, uint32_t crcinit)
{
    uint32_t crc;
